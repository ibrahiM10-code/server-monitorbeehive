...
from src.database.db_mongo import get_datos_sensores, update_datos_sensores, add_historial_sensores, get_ultimos_historial_sensores, get_historial_diario, get_expo_push_token, get_apicultor_by_colmena, get_colmena_info
...
def actualizar_sensores(colmena_id):
    ...
    try:
        ...
        print(update_fields)
        nombre_colmena = get_colmena_info(colmena_id)["nombre_colmena"]
        ...
        print("Apicultor ID y resultado: ",apicultor_id,resultado)
        if apicultor_id and resultado:
            ...
            if "expoPushToken" in expo_push_token[0].keys():
                ...
                generar_alerta(update_fields, colmena_id, nombre_colmena, apicultor_id, expo_push_token)
                print("Notificacion enviada al celular del apicultor!")
            else:
                generar_alerta(update_fields, colmena_id, nombre_colmena, apicultor_id)
